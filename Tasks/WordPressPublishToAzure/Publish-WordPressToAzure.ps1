...
Write-Host "Entering script Publish-WordPressToAzure.ps1"

Write-Verbose "WordPressPath = $WordPressPath"
...
$AzureTargetWebAppList | % {
    ...
    Write-Verbose "Starting publish of WordPress to $webappName"
    ...
    $webrootOutputFolder = (Get-Item $WordPressPath).FullName
    Write-Verbose "$webappName - webrootOutputFolder = $webrootOutputFolder"
    $publishArgs = @()
    $publishArgs += "-source:IisApp='$webrootOutputFolder'"
    $publishArgs += "-dest:IisApp='$msdeployIisAppPath',ComputerName='$msdeployComputerName',UserName='$msdeployUserName',Password='$msdeployPassword',IncludeAcls='False',AuthType='Basic'"
    ...
    Write-Verbose "Finished publish of WordPress to $webappName"
}


Write-Host "Leaving script Publish-WordPressToAzure.ps1"